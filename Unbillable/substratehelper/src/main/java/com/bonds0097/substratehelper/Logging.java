...
public class Logging {
    ...
    public static String GetMethodArguments(Object[] args) {
        ...
        for (int i = 0; i < args.length; i++) {
            ...
            } else if (args[i] instanceof android.content.Intent) {
                Intent intent = (Intent)args[i];
                Bundle extras = intent.getExtras();
                message.append(BundleToString(extras));
        }else {
    ...
    public static void HookingIntoCipher(String TAG, String methodName,
                                         Object _this, Object[] args, MS.MethodPointer old) {
        StringBuilder message = new StringBuilder();
    ...
    @SuppressWarnings({ "rawtypes", "unchecked" })
    public static void MethodReturn(String TAG, MS.MethodPointer old,
    ...
    public static void PrintBundle(String TAG, Object result) {
        if (result != null && result.getClass() == Bundle.class) {
            try {
                Bundle bundle = (Bundle) result;
                Log.d(TAG, BundleToString(bundle));
            } catch (Exception e) {
